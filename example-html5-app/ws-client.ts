...
import { handleWsMessage, initUtils, getMessageFromErrorResponse } from './ws-utils';
...
const createSocket = async () => {
  ...
  socket.on('connect_error', (error) => {
    if (socket.active) {
      ...
      console.log(`Socket connection has error but still active: ${getMessageFromErrorResponse(error)}`);
    } else {
      // the connection was denied by the server
      // in that case, `socket.connect()` must be manually called in order to reconnect
      console.log(`Socket connection failed: ${getMessageFromErrorResponse(error)}`);
    }
  });
