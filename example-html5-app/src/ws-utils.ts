...
const doGet = async (message: any, socket: Socket) => {
    try {
        ...
        const errMsg = getMessageFromErrorResponse(
            error,
...
const doPutOrPostOrDelete = async (
    message: any,
    reqMethod: string,
    socket: Socket
) => {
    try {
        ...
        if (
            message.payload.route.indexOf("/v1/files") >= 0 &&
            reqMethod === "PUT" &&
            ...
            for (const f of message.payload.data.files) {
                if (
                    f.fileContents &&
                    isDataURLWithBase64Encoding(f.fileContents)
                ) {
                    // make sure fileContents is a data URL
                    f.fileContents = Buffer.from(
                        f.fileContents.split(",")[1],
                        "base64"
                    );
                }
...
const getRouteUrl = (message: any) => {
    ...
    const route = pathJoin(
        `${config.dwsBaseRoute}:${dwsPort}/api`,
...
const fetchCall = async (
    ...
    if (isNonEmptyString(config.dwsPassword)) {
        const client = new DigestClient("admin", config.dwsPassword, {
            algorithm: "MD5",
        });
...
const sendWsResponse = function (
    ...
    try {
        if (socket && socket.connected) {
            const route = lodashGet(message, "payload.route")
                ? `for ${lodashGet(message, "payload.route")}`
                : "";
...
const handleRestRequest = (message: any, socket: Socket) => {
    if (message.payload && message.payload.route) {
        const routeNoTrailingSlash = message.payload.route.replace(
            /\/\s*$/,
            ""
        );
        ...
        message.payload.data = {
            error: "Required parameter missing in the payload: route",
        };
        sendWsResponse(message, socket);
    }
};
    ...
    if (message.source.type === "player" && message.source.name === serial) {
        ...
        if (message.destination.name === serial) {
            switch (message.type) {
                ...
                case "RestResponse": {
                    console.log(
                        `Received response from ${message.source.name}: ${JSON.stringify(message.payload)}`
                    );
                    // TODO: make sure the message does not broadcast back to self
